...
int eth_initialize( bd_t *bis ) {
    ...
    char valid_mac_string[32] = CONFIG_ETHADDR;
...
#define GDMA1_MAC_ADRH  0x30
			...
			if (magic == CFG_MAGIC)
			{
				...
				else if (!is_valid_ether_addr(rt2880_gmac1_mac))
				{
            		eth_parse_enetaddr( CONFIG_ETHADDR, rt2880_gmac1_mac );
				}
				else
				{
					sprintf(valid_mac_string, "%02x:%02x:%02x:%02x:%02x:%02x",
                            rt2880_gmac1_mac[0],rt2880_gmac1_mac[1],rt2880_gmac1_mac[2],
                            rt2880_gmac1_mac[3],rt2880_gmac1_mac[4],rt2880_gmac1_mac[5]);
					printf("Valid MAC address detected: %s\n", valid_mac_string);				
				}
			}
			else
			{
				printf("The device has not been encrypt!\n");
				eth_parse_enetaddr( CONFIG_ETHADDR, rt2880_gmac1_mac );
				printf("Using default MAC address [%s].\n", CONFIG_ETHADDR);
			}
			/* End */
			
			memcpy( dev->enetaddr, rt2880_gmac1_mac, 6 );
			/* Add by JINWEI@2015.9.1 */
			setenv("ethaddr", valid_mac_string);
